...
% Creates an analytical representation of the FORBILD phantom
% Input: oL = 1 to include the left ear insert, any other value to exclude it.
%        oR = 1 to include the right ear insert, any other value to exclude it.
% Output: phantom is a struct containing two fields:
%       phantom.E: matrix with rows giving parameters for each ellipse
%       phantom.C: matrix describing the "clipping lines" corresponding to
%       a few shapes that are truncated ellipses (e.g. shapes 14 and 15)
...
function [ phantom ] = analytical_phantom(oL, oR)
    ...
    for j = 1:3 
        kj = 8-2*floor(j/3); dj = 0.2*mod(j,2);
    ...
    if (oL==0&oR==0)    
        phantom.E=E(1:17,:); phantom.C=C(:,1:12);
    elseif(oL==0&oR==1) 
        phantom.E=[E;E_cavity]; phantom.C=C;
    elseif(oL==1&oR==0) 
        phantom.E=[leftear;E(1:17,:)]; phantom.C=C(:,1:12);
    else
        phantom.E=[leftear;E;E_cavity];phantom.C=C;
    end
