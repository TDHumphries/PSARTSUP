%% make_FORBILD_data_80kVp.m
% Code to generate 80 kvp data used in "Superiorized algorithm for reconstruction
...
xrs=xray_read_spectra('poly1,[80]'); % spectrum values
spectrum = xrs.sp/sum(xrs.sp); energies = xrs.en; %normalize spectrum to have integral of 1
mass_atten=xray_read_atten({'soft','bone'},[1:80]);
soft_vals = mass_atten(1:80,1)*1.06; %multiply by density to get linear attenuation coefficient; 
bone_vals = mass_atten(1:80,2)*1.92;

Eref = 50; %reference energy
...
%% Generate monoenergetic data. Since we use analytic line integrals, this produces data slightly inconsistent with the MIRT system matrix.
 ...
 img_50kev = discrete_phantom(xcoord,ycoord,P_phys); %generate discrete phantom at reference energy
...
figure(1); 
subplot(1,2,1); imagesc(img_50kev,win); axis image; colormap gray;
...
% 
 save FORBILD_data_80kVp img_50kev sino_mono_analytic sino_poly_analytic theta_deg numpix dx spectrum energies soft_vals bone_vals
...
save FORBILD_data_80kVp img_50kev sino_mono_analytic sino_poly_analytic theta_deg numpix dx spectrum energies soft_vals bone_vals sino_mono_noisy sino_poly_noisy

%% generate reduced spectrum used for inconsistent data reconstructions
load FORBILD_data_80kVp

figure(1); clf; plot(energies,spectrum,'b-'); hold on;
selectedenergies = [4 5 10 11 12 16 20 24 28 32 40 50 56:60 66:70 80]; %specifically chosen energies from the spectrum
...
%% generate exact data. Matlab's radon command is used to generate the data, and attenuation exactly follows the model of pSART
load FORBILD_data_80kVp
...
for j = 1:numel(theta_deg)
    ...
    sino_mono_mirt(:,j) = exp(-A*img_50kev(:)); 
    sino_poly_mirt(:,j) = project_poly(img_50kev,Eref,energies,spectrum,material_att,material_names,A,[dims(1) 1],false);
end


save FORBILD_data_80kVp img_50kev sino_mono_analytic sino_poly_analytic theta_deg numpix dx spectrum* energies* soft_vals bone_vals sino_mono_noisy sino_poly_noisy sino_poly_mirt sino_mono_mirt
